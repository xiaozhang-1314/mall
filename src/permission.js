 ...
 */
 ...
 router.beforeEach((to, from , next) => {
    // 1. 获取token
    const token = localStorage.getItem('xiaoxueshen') 
    if(!token) {
        // 1.1 如果没有获取到，
        ...
        if(to.path === '/login') {
            ...
            const userInfo = localStorage.getItem('zhongxueshen')
            if(userInfo) {
                // 本地获取到，则直接让它去目标路由
                next()
            }else {
                ...
                getUserInfo(token).then(response => {
                    ...
                    if(resp.flag) {
                        ...
                        // 保存到本地
                        localStorage.setItem('zhongxueshen', JSON.stringify(resp.data))
                        next()
                    }else {
                        // 未获取到用户信息，重新登录 
                        next({path: '/login'})
